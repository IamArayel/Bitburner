...
function fileFilter(file) {
    ...
    if (config.get("exclude").some((x) => isSubDirOf(file.path, x)))
        return false;
...
export async function setupWatch(signaller) {
    ...
    if (!config.get("quiet"))
        console.log("Watching folder", resolve(config.get("scriptsFolder")));
    watch.on("+", (fileEvent) => {
        if (fileEvent.stats.isFile())
            signaller.emit(EventType.FileChanged, fileEvent);
    });
    watch.on("-", (fileEvent) => {
        if (fileEvent.stats.isFile())
            signaller.emit(EventType.FileDeleted, fileEvent);
    });
